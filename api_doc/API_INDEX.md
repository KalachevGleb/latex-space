...
Однако эти ресурсы относятся к основному репозиторию Overleaf, а не к данному Fork'у, там не может быть ответов на вопросы относительно добавленного функционала. Кроме того, следует учитывать, что данный fork не синхронизируется с основным репозиторием, а развивается отдельно, поэтому информация об исходной версии Overleaf может стать не актуальной для данной версии через некоторое время.
