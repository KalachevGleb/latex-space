...
Создаёт аккаунт без отправки письма: e-mail сразу помечается подтверждённым,
пароль не задаётся (вход через браузер невозможен, аккаунт используется через
Service API с заголовком `X-Overleaf-User-Id` или `X-Overleaf-User-Email`).
E-mail должен быть синтаксически корректным, но не обязан существовать.
...
|------|-----|-------------|----------|
| `email` | string | да | E-mail нового пользователя (уникальный) |
| `first_name` | string | нет | Имя; по умолчанию — часть e-mail до `@` |
| `last_name` | string | нет | Фамилия; по умолчанию пустая |

Имя и фамилия показываются везде, где отображается автор (комментарии, track
changes, список участников), если для проекта не задан псевдоним
(см. «Задать псевдоним участника»).
...
Возвращает плоский список документов и файлов проекта, отсортированный по пути.
`id` документа используется в endpoints вида `/project/:Project_id/doc/:doc_id/...`.
...
```json
{
  "project_id": "60a7b1234567890abcdef123",
  "entities": [
    { "id": "6a8f58a28835f5beb22d9154", "path": "/main.tex", "type": "doc" },
    { "id": "6a8f58a28835f5beb22d9160", "path": "/figures/plot.png", "type": "file" }
  ]
}
```
...
| Поле | Описание |
|------|----------|
| `id` | Идентификатор документа (`doc_id`) или файла (`file_id`) |
| `path` | Путь от корня проекта |
| `type` | `doc` — текстовый документ, `file` — бинарный файл |
...
Перед чтением несохранённые изменения сбрасываются из document-updater в
хранилище, поэтому ответ отражает текущее состояние документов.
...
Удаляет тред вместе со всеми сообщениями и убирает привязанный к нему диапазон
из документа. Если комментарий находится не в документе `:Doc_id`, документ
ищется по всему проекту.
...
Добавление комментариев и правок в документ от имени пользователя, указанного в
`X-Overleaf-User-Id` / `X-Overleaf-User-Email`. Правки отображаются в редакторе
как track changes (их можно принять или отклонить), комментарии — в панели
рецензирования; автором является указанный пользователь.

**Требования к пользователю:** участник проекта с ролью `owner`, `readAndWrite`
или `review`. Добавить участника: `POST /service/project/:Project_id/add`.

**Позиции:** `pos` — смещение в символах от начала документа; строки документа
соединены символом `\n`. Текущий текст документа: `GET /service/Project/:Project_id/doc/:doc_id/download`,
идентификатор документа: `GET /service/project/:Project_id/entities`. Указанный
в запросе текст (`text`, `old_text`) сверяется с документом по позиции; при
расхождении возвращается `409 text_mismatch`.
...
| Поле | Тип | Обязательно | Описание |
|------|-----|-------------|----------|
| `pos` | number | да | Смещение начала фрагмента |
| `text` | string | да | Текст фрагмента, к которому привязывается комментарий; должен совпадать с текстом документа начиная с `pos` |
| `content` | string | да | Текст комментария (Markdown и формулы MathJax, как в обычных комментариях) |
| `author_alias` | string \| null | нет | Установить псевдоним автора в этом проекте (см. «Задать псевдоним участника»); `null` или `""` — удалить псевдоним |
...
| Поле | Описание |
|------|----------|
| `thread_id` | Идентификатор треда; используется в `/project/:Project_id/thread/:thread_id/...` |
| `position` | Границы фрагмента в документе (`end` не включается) |
| `message` | Созданное сообщение и его автор |

### Добавить правки как track changes

Применяет одну или несколько замен как tracked changes от имени пользователя.
Каждый элемент `items` заменяет фрагмент `old_text`, начинающийся с `pos`, на
`new_text`. Diff строится по словам: в track changes попадают только
изменившиеся слова. Если у элемента задан `comment`, к новому тексту
привязывается комментарий (при пустом `new_text` — к символу сразу после
...
| Поле | Тип | Обязательно | Описание |
|------|-----|-------------|----------|
| `items` | array | да | Список замен, от 1 до 500 элементов |
| `items[].pos` | number | да | Смещение начала заменяемого фрагмента в **текущем** документе (до применения других элементов) |
| `items[].old_text` | string | да | Заменяемый текст; должен совпадать с текстом документа начиная с `pos` |
| `items[].new_text` | string | да | Новый текст; `""` — удаление. Должен отличаться от `old_text` |
| `items[].comment` | string | нет | Текст комментария к этой правке |
| `author_alias` | string \| null | нет | Установить псевдоним автора в этом проекте; `null` или `""` — удалить |

Фрагменты `items` не должны пересекаться. Порядок элементов не важен.
...
| Поле | Описание |
|------|----------|
| `version` | Версия документа после применения правок |
| `applied` | Число применённых элементов |
| `comments` | Созданные треды комментариев: `index` — индекс элемента в исходном массиве `items`, `thread_id` — идентификатор треда |

Правки принимаются и отклоняются как обычные track changes (в редакторе или
через `POST /project/:Project_id/doc/:doc_id/changes/accept`). Включать режим
track changes в проекте не требуется.

**Коды ошибок (оба endpoint):**

| Код | `error` | Причина |
|-----|---------|---------|
| 400 | `invalid_pos`, `invalid_text`, `invalid_content`, `invalid_items`, `too_many_items`, `no_change`, `overlapping_items`, `invalid_comment`, `invalid_alias` | Некорректное тело запроса; для ошибок в `items` в ответе есть `index` |
| 403 | — | Пользователь не имеет прав на запись/рецензирование проекта |
| 404 | `not_found` | Документ не найден |
| 409 | `text_mismatch` | Текст по позиции не совпадает с `text`/`old_text`; в ответе `expected`, `actual` (и `index` для `items`) |
| 409 | `ops_rejected` | Документ изменился между чтением и записью; перечитайте документ и повторите запрос |

### Задать псевдоним участника

Устанавливает или удаляет псевдоним участника проекта. Псевдоним заменяет имя и
фамилию пользователя везде, где показывается автор в этом проекте: комментарии,
track changes, список участников. Открытые редакторы обновляют имя сразу.

```http
PUT /service/api/project/:Project_id/users/:user_id/alias
Authorization: Basic overleaf:<service_password>
X-Overleaf-User-Id: <owner_user_id>
Content-Type: application/json
...
| Поле | Тип | Обязательно | Описание |
|------|-----|-------------|----------|
| `alias` | string \| null | да | Псевдоним (до 100 символов); `null` или `""` — удалить псевдоним |

Требуются права владельца проекта. Свой собственный псевдоним участник может
задать полем `author_alias` в запросах выше.

**Ответ при успехе (200):**
```json
{ "user_id": "6a8f5121a48c86a4e999addc", "alias": "ИИ корректура" }
```
...
| Код | `error` | Причина |
|-----|---------|---------|
| 400 | `missing_alias`, `invalid_alias` | Поле `alias` отсутствует или некорректно |
| 403 | — | Пользователь не владелец проекта |
| 404 | `not_a_member` | `user_id` не является участником проекта |

### Пример: последовательность вызовов

```bash
BASE=http://localhost; AUTH="-u overleaf:$SERVICE_PASSWORD"

# 1. Создать бот-аккаунт (один раз)
...
# 2. Добавить бота в проект как рецензента (от имени владельца)
...
# 3. Получить id документа и его текст
DOC_ID=$(curl -s $AUTH -H "X-Overleaf-User-Id: $BOT" \
  $BASE/service/project/$PROJECT_ID/entities | jq -r '.entities[] | select(.path=="/main.tex") | .id')
curl -s $AUTH -H "X-Overleaf-User-Id: $BOT" \
  $BASE/service/Project/$PROJECT_ID/doc/$DOC_ID/download > main.tex

# 4. Отправить комментарий и правки от имени бота
curl -s $AUTH -H "X-Overleaf-User-Id: $BOT" -H 'Content-Type: application/json' \
  -d '{"pos":99,"text":"cellular automata","content":"Уточните класс автоматов.","author_alias":"ИИ рецензия"}' \
  $BASE/service/api/project/$PROJECT_ID/doc/$DOC_ID/comments
curl -s $AUTH -H "X-Overleaf-User-Id: $BOT" -H 'Content-Type: application/json' \
  -d '{"items":[{"pos":75,"old_text":"studys","new_text":"studies","comment":"Опечатка."}]}' \
